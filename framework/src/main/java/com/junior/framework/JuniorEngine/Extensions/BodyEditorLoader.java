...
public class BodyEditorLoader {
    ...
    public Array<Array<Vector2[]>> retrieveAnimationShapes(String name, float scale) {
        RigidBodyModel rbModel;
        rbModel = model.rigidBodies.get(name);

        if(!(rbModel == null)){
            Array<Array<Vector2[]>> singleAnimationArray = new Array<>(1);
            Array<Vector2[]> fixtureShapes = new Array<>();
            ...
            for (int i = 0, n = rbModel.polygons.size(); i < n; i++) {
                ...
                fixtureShapes.add(vertices);
            }

            singleAnimationArray.add(fixtureShapes);
            return singleAnimationArray;
        }

        rbModel = model.rigidBodies.get(name + "1");
        if (rbModel == null) throw new RuntimeException("Name '" + name + "' animation was not found.");

        Array<Array<Vector2[]>> animationShapeList = new Array<>();
        ...
        animationShapeList.add(fixtureShapes);

        for(int j =2;; j++) {
            RigidBodyModel rb1Model = model.rigidBodies.get(name + j);
            try {
                if (rb1Model == null) throw new Exception();

                Array<Vector2[]> shapes = new Array<>();

                Vector2 orgn = vec.set(rbModel.origin).scl(scale);
                ...
                for (int i = 0, n = rbModel.polygons.size(); i < n; i++) {
                    ...
                    for (int ii = 0, nn = vertices.length; ii < nn; ii++) {
                        vertices[ii] = newVec().set(polygon.vertices.get(ii)).scl(scale);
                        vertices[ii].sub(orgn);
                    }

                    shapes.add(vertices);
                }

                animationShapeList.add(shapes);
            }
            catch(Exception e){
                break;
            }
        }
        return animationShapeList;
    }

    public Array<Array<Vector2[]>> retrieveAnimationShapes(String name){
        return retrieveAnimationShapes(name, 1);
    }
